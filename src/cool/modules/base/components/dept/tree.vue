	...
	<div class="cl-dept-tree">
		<div class="cl-dept-tree__header">
			...
			<ul class="cl-dept-tree__op">
				...
				<li v-if="drag && !browser.isMini" v-permission="'dept@sort'">
...
import { ContextMenu, Form } from "@/cool/lib/cl-admin-crud";
...
export default {
	...
	methods: {
				...
				list: [
					{
						...
						hidden: n && n.level >= this.level || !checkPerm("dept@create"),
						callback: (_, done) => {
							this.rowEdit({
								name: "",
								parent_name: d.name,
					...
					},
					{
						...
						hidden: !Boolean(d.parent_id) || !checkPerm("dept@delete"),
						callback: (_, done) => {
							this.rowDel(d);
							done();
						}
		...
		rowDel(e) {
			const del = () => {
				this.$http.post("dept/delete", { id: e.id })
					.then(() => {
			...
			this.$confirm(`该操作会删除本部门及下属部门且转移所有用户到上级,是否确认？`, "提示", {
				type: "warning",
