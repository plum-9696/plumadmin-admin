...
export default {
	...
	methods: {
		output() {
			if (this.multiple) {
				const urls = this.files.map(e => e.url);
				this.$emit("input", urls);
			} else {
				const url = this.files.length === 0 ? '' : this.files[0].url;
				this.$emit("input", url);
			}
		},
		//构建数组
		...
		isChange(val) {
			val = clone(val);
			return val.sort().join() !== this.files.sort().join();
		},
		//插入
		entry(val) {
			//构建成数组
			let value = cloneDeep(this.builder(val))
			//有变动,获取服务器数据,插入渲染
			if (this.isChange(value)) {
				this.files = value.map(e=>{
					return {url:e,file_type:this.type,status:'success'}
				})
			}
		},
		deleteFile(index) {
